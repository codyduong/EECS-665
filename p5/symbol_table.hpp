#ifndef A_LONG_SYMBOL_TABLE_HPP
#define A_LONG_SYMBOL_TABLE_HPP
#include <string>
...
class CustomSymbol : public SemSymbol{
public:
	CustomSymbol(std::string name, ScopeTable * fields)
	: SemSymbol(name, new ClassType(name, fields)), myFields(fields){ }
	virtual SymbolKind getKind() const { return AGG; }
	SymbolKind getKind(){ return AGG; }
	SemSymbol * fieldSymbol(std::string name);
private:
	ScopeTable * myFields;
};
...
class ScopeTable {
	public:
		ScopeTable();
		SemSymbol * lookup(std::string name) const;
		...
		SemSymbol * addClass(std::string name, ScopeTable * fields){
			CustomSymbol * sym = new CustomSymbol(name, fields);
			insert(sym);
			return sym;
		}
	private:
...
class SymbolTable{
	public:
		...
		SemSymbol * addClass(std::string name, ScopeTable * fields){
			return getCurrentScope()->addClass(name, fields);
		}
		void print();
	private:
