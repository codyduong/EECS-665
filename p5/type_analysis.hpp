...
public:
	...
	void errFileEq(const Position * pos){
		hasError = true;
		Report::fatal(pos, 
			"Equality operator applied to array variables");
	}
	void errArrayEq(const Position * pos){
		hasError = true;
		Report::fatal(pos, 
			"Equality operator applied to array variables");
	}
	void badCallee(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to call a "
			"non-function");
	}
	void badArgCount(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Function call with wrong"
			" number of args");
	}
	void badArgMatch(const Position * pos){
		hasError = true;
		Report::fatal(pos, 
			"Type of actual does not match"
			" type of formal");
	}
	void badRetValue(const Position * pos){
		hasError = true;
		Report::fatal(pos, 
			"Bad return value");
	}
	void badNoRet(const Position * pos){
		hasError = true;
		Report::fatal(pos, 
			"Missing return value");
	}
	void badReadFn(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to read a function");
	}
	void badWritePtr(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to write a raw pointer");
	}
	void badWriteFn(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to output a function");
	}
	void badWriteVoid(const Position * pos){
		hasError = true;
		Report::fatal(pos, 
			"Attempt to write void");
	}
	void testErrorType(const Position * pos){
		hasError = true;
		Report::fatal(pos,
		"Found an error here");
	}
